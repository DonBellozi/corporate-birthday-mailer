...
DEFAULTS = {
    ...
    # Названия подразделений, которые пропускаются при поиске ближайшего
    # "содержательного" подразделения для текста поздравления - сами по
    # себе они не говорят, где именно работает человек ("Администрация"
    # есть почти в каждом филиале). По одному названию (или началу
    # названия) на строку. В дереве на странице "Должности" такие уровни
    # по-прежнему видны - пропускается только при сборке текста письма.
    "position_skip_units": "администрация\nруководство",
